...
`SortableItem` 表示值中必须要有 `id` 属性作为唯一键值。

| 属性名             | 类型                                                                        | 描述                               |
| ------------------ | --------------------------------------------------------------------------- | ---------------------------------- |
| value              | `SortableItem[]`                                                            | 值                                 |
| initialValues      | `SortableItem[]`                                                            | 初始值                             |
| onChange           | `(value: SortableItem[], event: ListDataDispatchPayload) => void`           | 值变化                             |
| renderContent      | `(item: SortableItem, index: number) => ReactNode`                          | 自定义可排序列表项内容             |
| renderItem         | `(item: SortableItem, options) => ReactNode`                                | 自定义可排序列表项                 |
| getItemStyle       | `(status: GetItemStylesArgs) => ReactNode`                                  | 自定义容器样式                     |
| ref                | `ForwardedRef<SortableListRef<SortableItem>>`                               | 对外部暴露方法                     |
| hideRemove         | `boolean`                                                                   | 是否隐藏删除按钮，默认为 false     |
| creatorButtonProps | `CreatorButtonProps\|false`                                                 | 新建对象相关属性                   |
| actions            | `(item: SortableItem, index: number) => ReactNode[]` \| `React.ReactNode[]` | 除列表自带操作之外的其他操作自渲染 |

### CreatorButtonProps 创建按钮属性

新增一行的时候要保证 `creatorButtonProps.record` id 值唯一，不然会导致组件逻辑出错。
